...
export default function useProducts() {
    ...
    async function addProduct(product: Product, res_name: string) {
        const docRef = await addDoc(collection(db, `XInYuan/${res_name}/menu`),
    ...
    async function deleteProduct(id: string, res_name: string) {
        try {
            const db = getFirestore(app);
            await deleteDoc(doc(db, `XinYuan/${res_name}/menu`, id));
            setUpdated((currentValue) => currentValue + 1)
        }
        catch (error) {
            console.error(error);
    ...
    async function updateProduct(product: Product, res_name: string) {
        try {
            const db = getFirestore(app);
            await updateDoc(doc(db, `XinYuan/${res_name}/menu`, product.id),
