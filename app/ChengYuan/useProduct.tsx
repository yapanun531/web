...
export default function useProducts() {
    ...
    useEffect(() => {
        async function fetchData() {
            setIsLoading(true);
            const querySnapshop = await getDocs(collection(db, "ChengYuan"));
            ...
            for (const shop of querySnapshop.docs) {
                const querySnapshotMenu = await getDocs(collection(db, "ChengYuan/" + shop.id + "/menu"));
    ...
    async function addProduct(product: Product, res_name: string) {
        const docRef = await addDoc(collection(db, `ChengYuan/${res_name}/menu`),
    ...
    async function deleteProduct(id: string, res_name: string) {
        try {
            const db = getFirestore(app);
            await deleteDoc(doc(db, `ChengYuan/${res_name}/menu`, id));
            setUpdated((currentValue) => currentValue + 1)
        }
        catch (error) {
            console.error(error);
    ...
    async function updateProduct(product: Product, res_name: string) {
        try {
            const db = getFirestore(app);
            await updateDoc(doc(db, `ChengYuan/${res_name}/menu`, product.id),
